...
Object.entries(data.directoryGroups).forEach(([groupName, nodeIds]) => {
  nodeIds.forEach(nodeId => {
    if (mappedIds.has(nodeId)) return;

    const nodeType = nodeId.split(':')[0];

    // Priority to non-code types
    if (nodeType === 'table' || nodeType === 'schema' || nodeId.includes('infra/db/')) {
      ...
      // Use directory group name to assign code nodes
      if (groupName === 'access-core') {
        layers.find(l => l.id === 'layer:access-core').nodeIds.push(nodeId);
      } else if (groupName === 'asset-core') {
