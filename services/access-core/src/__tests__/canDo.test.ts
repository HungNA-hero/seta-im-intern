...
import { canDo, canDoWithKnownAncestors } from "../authz/decision";
import { authorizationService, resetInProcessAuthzCachesForTests } from "../authz/authorizationRoot";

const filterAllowedResourceIds = authorizationService.filterAllowedResourceIds;
...
describe("permActionCache", () => {
  test("calls permissionAction.findMany only once across multiple canDo calls", async () => {
    ...
    await canDo({ userId: "user-1", action: "write", resourceType: "folder", resourceId: "f2", orgId: "org-1" });
...
describe("early exits (before DB permission checks)", () => {
  ...
  test("denies when action code is not in the permissionAction table", async () => {
    ...
    const result2 = await canDo({
      userId: "user-1",
      action: "invalid_code" as any,
      resourceType: "folder",
      resourceId: "f1",
      orgId: "org-1",
    });
...
describe("tombstone restore authorization", () => {
  ...
  test("uses the trusted ancestor fact to honor a current inherited write grant", async () => {
    ...
    mockPrisma.objectPermission.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([{ resourceId: rootId }]);
...
describe("manage_permissions never inherits (OLP mode)", () => {
  ...
  test("a grant on an ancestor folder does NOT satisfy manage_permissions for a descendant", async () => {
    ...
    mockPrisma.objectPermission.findMany.mockImplementation(
      async ({ where }: { where: { resourceId: { in: string[] } } }) =>
        where.resourceId.in.includes(rootId) ? [{ resourceId: rootId }] : [],
    );
  ...
  test("filterAllowedResourceIds does not fall back to ancestor grants for manage_permissions", async () => {
    ...
    const result = await filterAllowedResourceIds({
      ...
      resourceIds: ["child-1"],
      items: [{ id: "child-1" }],
      getHierarchy: () => ({ ancestorIds: ["ancestor-1"] }),
    });
