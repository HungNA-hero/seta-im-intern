...
describe("media REST routes", () => {
  ...
  test("does not expose status polling before Asset Core owns a working handler", async () => {
    const { app } = await buildHarness();
    const response = await app.inject({ method: "GET", url: `/api/v1/assets/${assetId}/media/status` });
