...
describe("assertResourceInOrg", () => {
  ...
  test("maps a missing metadata target to METADATA_NOT_FOUND", async () => {
    ...
    await expect(assertResourceInOrg("metadata_item", "metadata-1", "org-1", "user-1")).rejects.toMatchObject({
      extensions: { code: "METADATA_NOT_FOUND" },
    });
  });
  ...
  test("maps a missing folder target to FOLDER_NOT_FOUND", async () => {
    ...
    await expect(assertResourceInOrg("folder", "folder-1", "org-1", "user-1")).rejects.toMatchObject({
