...
export function rethrowPrismaError(
  err: unknown,
  map: Partial<Record<"P2002" | "P2025", string>>,
): never {
  const code = (err as any)?.code as string | undefined;
  if (code === "P2002" && map.P2002)
    throw new GraphQLError(map.P2002, { extensions: { code: "CONFLICT" } });
  if (code === "P2025" && map.P2025)
    throw new GraphQLError(map.P2025, { extensions: { code: "NOT_FOUND" } });
  throw err;
}
