...
// authorizeFolderRestore evaluates current policy from a trusted tombstone fact.
// The fact is not a public query and carries no display data.
export async function authorizeFolderRestore(
  ctx: GraphQLContext,
  orgId: string,
  folderId: string,
): Promise<void> {
  ...
  await assertCurrentRestorePermission(
    ...
    "folder",
    fact.id,
    ancestorIdsFromPath(fact.path),
  );
}

// authorizeMetadataRestore evaluates metadata grants and inherited folder grants
// from the trusted metadata-to-folder relationship at restore time.
export async function authorizeMetadataRestore(
  ctx: GraphQLContext,
  orgId: string,
  metadataId: string,
): Promise<void> {
  ...
  await assertCurrentRestorePermission(
    ...
    "metadata_item",
    fact.id,
    [fact.folderId, ...ancestorIdsFromPath(fact.folderPath)],
  );
}
