...
Options:
  ...
  --run-id ID              Correlation ID for this run (generated by default)
  --help                   Show this help
EOF
}
...
while (($# > 0)); do
  case "$1" in
    --scenario|--base-url|--k6-base-url|--user-id|--org-id|--folder-id|--max-vus|--sleep-seconds|--ramp-up|--hold-duration|--ramp-down|--page-size|--total-requests|--target-rps|--pre-allocated-vus|--run-id)
      if (($# < 2)); then
...
if [[ ! "$run_id" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$ ]]; then
  echo "Run ID must be 1-64 characters using letters, digits, dot, underscore, or hyphen." >&2
  exit 2
fi
...
preflight_body="$(printf '{"query":"query LoadPreflight($orgId: ID!) { folderTree(orgId: $orgId) { id } }","variables":{"orgId":"%s"}}' "$org_id")"
if ! preflight_response="$(curl --fail --silent --show-error --connect-timeout 5 --max-time 15 \
  --header "Content-Type: application/json" \
  --header "x-user-id: ${user_id}" \
  --header "x-org-id: ${org_id}" \
  --data "$preflight_body" \
  "${base_url}/graphql")"; then
  echo "GraphQL preflight failed at ${base_url}/graphql. Run the migrations and demo seeds before load testing." >&2
  exit 1
fi
if [[ "$preflight_response" != *'"data"'* || "$preflight_response" == *'"errors"'* ]]; then
  echo "GraphQL preflight returned an application error. Run 'make migrate' and seed the demo fixtures documented in scripts/load/README.md." >&2
  exit 1
fi

timestamp="$(date +%Y%m%d-%H%M%S)"
summary_file="/results/${timestamp}-${run_id}-${scenario}-summary.json"

echo "Running ${scenario} (run ID ${run_id}) against ${k6_base_url} with at most ${max_vus} VUs."
docker run --rm -i \
